...
import { getPageById, getAllPages } from "@/lib/queries"
...
export default async function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [page, allPages] = await Promise.all([
    isNew ? Promise.resolve(null) : getPageById(Number(id)),
    getAllPages(),
  ])
  ...
  return (
    <div className="space-y-6">
      ...
      <PageEditor page={page} allPages={pageOptions} />
    </div>
  )
}
